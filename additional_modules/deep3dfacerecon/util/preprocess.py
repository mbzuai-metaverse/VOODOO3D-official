...
def resize_n_crop_img(img, lm, lm_orig, t, s, target_size=224., mask=None):
    w0, h0 = img.size
    ...
    lm_orig[..., 0] *= w / float(w0)
    lm_orig[..., 1] *= h / float(h0)
...
# utils for face reconstruction
def align_img(img, lm, lm_orig, lm3D, mask=None, target_size=224., rescale_factor=102.):
    """
    Return:
    ...
    img_new, lm_new, lm_orig_new, mask_new = resize_n_crop_img(img, lm, lm_orig, t, s, target_size=target_size, mask=mask)
    trans_params = np.array([w0, h0, s, t[0], t[1]])

    return trans_params, img_new, lm_new, lm_orig_new, mask_new, lm5p
